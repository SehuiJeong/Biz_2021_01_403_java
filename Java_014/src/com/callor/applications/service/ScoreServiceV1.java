...
public class ScoreServiceV1 {
	...
	public void init() {
		...
		// scanner와 Random을 선언하는 것과 분리하여
		// 생성하는 코드를 작성했다
		scan = new Scanner(System.in);
		rnd = new Random();

		System.out.println("몇명의 학생의 성적을 처리할까요");
		System.out.print("학생수 >> ");
		String strNums = scan.nextLine();
		int intNums = Integer.valueOf(strNums);

		// 입력받은 학생 수만큼 성적입력 배열 생성
	...
	public void inputScore() {
		for (int i = 0; i < strName.length; i++) {
			...
			intKor[i] = Integer.valueOf(strEng);

			System.out.print("수학");
			String strMath = scan.nextLine();
			intKor[i] = Integer.valueOf(strMath);
		}
	}
	...
	public void makeNamesAndArrayInit() {
		...
		strName = new String[] { "멧돼지", "원숭이", "사슴", "새", "왕", "고양이", "건", "장윤정" };
