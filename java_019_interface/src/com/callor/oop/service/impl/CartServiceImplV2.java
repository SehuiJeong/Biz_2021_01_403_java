...
public class CartServiceImplV2 implements CartService {
	 ...
	 * 생성을 하지 않는다.
	 * 
	 * 이렇게 하면 생성되지 않은 인스턴스 객체변수의 method를 호출하는
	 ...
	 * 		작동되는 과정에서 혹시 모를 메모리 누수(leak)를 방지하기 위한 조치
	 * 
	 * Java GC : 프로젝트가 실행되는 과정에서
	 * 			사용이 끝났음에도 아직 메모리에 남아 있는
	 * 			변수, 객체 등을 자동으로 제거하여
	 * 			메모리 누수를 없애고, 관리하는 Java의 기능
	 */
	 ...
	 *  클래스 이름하고 같고 첫글자 대문자부터 소문자까지 같은 method
	 *  return type(void, int, String)이 없는 method
	 * 
	 *  클래스를 객체로 선언만하고, 초기화(생성)를 할 때 사용하는 아래 코드에서
	 *  CartServiceImplV2 cs = new CartServiceImplV2();
	 *  
	 *  new CartServiceImplV2() 이 부분은 코드가
